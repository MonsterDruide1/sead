...
ExpHeap* ExpHeap::create(size_t size, const SafeString& name, Heap* parent, s32 alignment, HeapDirection direction, bool enableLock)
{
    ExpHeap* heap = ExpHeap::tryCreate(size, name, parent, alignment, direction, enableLock);
...
ExpHeap* ExpHeap::tryCreate(size_t size_, const SafeString& name, Heap* parent, s32 alignment, HeapDirection direction, bool enableLock)
{
    if (!parent)
    {
        parent = HeapMgr::instance()->getCurrentHeap();
...
ExpHeap* ExpHeap::tryCreate(void* start, size_t size, const SafeString& name, bool enableLock)
{
    ...
    ExpHeap* heap = new(start) ExpHeap(name, nullptr, start, size, HeapDirection::cHeapDirection_Forward, enableLock);
...
size_t ExpHeap::adjust()
{
...

    {
        ...
        if (mDirection == HeapDirection::cHeapDirection_Forward)
            newSize = adjustBack_();
        else
...
size_t ExpHeap::getFreeSize() const
{
    ...
    ConditionalScopedLock<CriticalSection> lock((CriticalSection*)&mCS, isEnableLock());
...
size_t ExpHeap::getMaxAllocatableSize(s32 alignment) const
{
    ...
    return block->getSize() - diff - 0x18;
}
...
void ExpHeap::dumpYAML(WriteStream& stream, s32 indent) const
{
    ...
    if (getAllocMode() == AllocMode::FirstFit)
        allocMode = "First Fit";
    else
        allocMode = "Best Fit";
...
void ExpHeap::createMaxSizeFreeMemBlock_(ExpHeap* heap)
{
    ...
    MemBlock* block = nullptr;
    if (heap->mDirection == HeapDirection::cHeapDirection_Forward)
        block = new(PtrUtil::addOffset(heap, sizeof(ExpHeap) + 8)) MemBlock();   // +8 for alignment to 16-bytes, as ExpHeap is 0x118 in size
    else
